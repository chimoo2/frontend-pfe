...
export default function RequiredSkillsList({ skills = [], onChange }) {
  ...
  const addSkill = () => {
    onChange && onChange([...skills, { skillName: '', domain: '', family: '', category: '', type: '', criticality: '' }]);
  };
    ...
    <div className="cp-section">
      ...
      {skills.length > 0 ? (
        <div className="cp-table-wrap">
          <table className="cp-table">
            ...
            <tbody>
              {skills.map((s, i) => (
                <tr key={i}>
                  <td><input className="cp-table-input" value={s.skillName || ''} onChange={handleFieldChange(i, 'skillName')} placeholder="Name" /></td>
                  ...
                  <td>
                    <button className="cp-btn-icon-sm cp-btn-danger-sm" onClick={() => removeSkill(i)} title="Delete">
        ...
        <div className="cp-empty">
          ...
          <p style={{margin:0, fontWeight:600, color:'#64748b'}}>No skills added</p>
          <span style={{fontSize:'0.8rem', color:'#94a3b8'}}>Use the selector above or add manually</span>
        </div>
      )}
    </div>
  );
